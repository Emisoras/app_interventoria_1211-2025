// src/components/checklist-form-instalacion-junta.tsx
'use client';
...
import { getChecklistById, getUserById, runComplianceCheck, saveChecklist, getContractors, addContractor, updateContractor, deleteContractor, getCampuses, addCampus, updateCampus, deleteCampus, type ChecklistQuestion } from '@/app/actions';
...
import { checklistInstalacionJuntaInternetData } from '@/lib/checklist-data';
...
const formSchema = z.object({
  ...
  institutionName: z.string(), // Not required for Junta
  campusName: z.string().min(1, 'El nombre de la junta es requerido.'),
...
const checklists = {
  junta: checklistInstalacionJuntaInternetData.map((item) => ({
    ...item,
    observation: '',
    photoDataUri: null,
    status: 'na',
...
const initialFormState = {
    ...
    municipality: '',
    date: new Date(),
    siteType: 'Junta de Internet',
    inspectorName: '',
    _id: undefined,
};

export function ChecklistFormInstalacionJunta({ isViewer }: { isViewer: boolean }) {
  const { toast } = useToast();
  const searchParams = useSearchParams();
  const router = useRouter();
  
  const [checklistItems, setChecklistItems] = React.useState<ChecklistItem[]>(checklists.junta);
  ...
  const [newCampus, setNewCampus] = React.useState({ name: '', municipality: ''});
  ...
  const getViabilityReportDefaultContent = (formData: ContractorFormData) => {
    const institutionText = `la Junta de Internet ${formData.campusName}`;
    return {
      antecedentes: `Dentro del marco de ejecución del Convenio Interadministrativo No. 1211-2025, y conforme a las funciones asignadas a la interventoría técnica, se realizó la revisión del Estudio de Campo para ${institutionText} con el fin de realizar la aprobación del mismo. Durante esta revisión se identificaron aspectos que requieren análisis técnico, los cuales se detallan a continuación.`,
      analisis: `Con base en la revisión documental, se evidenció lo siguiente:\n• El contratista realizo a satisfacción el Estudio de Campo para ${institutionText} cumpliendo los Ítems 1.7 del Anexo Técnico.`,
      conclusion: `• Se concluye que se aprueba por parte de Interventoría el Estudio de Campo para ${institutionText} cumpliendo los Ítems 1.7 del Anexo Técnico con concepto de viabilidad positivo por lo cual se puede proceder con la Fase de Instalación.`
    };
  };
  ...
  const [viabilityConclusion, setViabilityConclusion] = React.useState('');
    ...
    const fetchDropdownData = React.useCallback(async () => {
        const [contractorsData, campusesData] = await Promise.all([
            getContractors(),
            getCampuses(),
        ]);
  ...
  React.useEffect(() => {
    ...
    const loadData = async () => {
        ...
        if (checklistId) {
            ...
            if (data && data.siteType === 'Junta de Internet') { // Only load if it's the correct type
                const formData = { ...data, date: new Date(data.date) };
                form.reset(formData);
                ...
                setViabilityAntecedentes(data.viabilityAntecedentes || defaults.antecedentes);
                setViabilityAnalisis(data.viabilityAnalisis || defaults.analisis);
                setViabilityConclusion(data.viabilityConclusion || defaults.conclusion);
            }
        } else {
  ...
  }, [searchParams, fetchDropdownData]);
  ...
  const handleAddCampus = async () => {
    if (newCampus.name.trim()) {
      const result = await addCampus({ ...newCampus, institutionName: '' }); // institutionName is not relevant for juntas
      if (result.success) {
        setCampuses([...campuses, result.campus]);
        setNewCampus({ name: '', municipality: '' });
      } else {
        toast({ variant: 'destructive', title: 'Error', description: result.error });
      }
    }
  ...
  const handleUpdateCampus = async () => {
    if (editingCampus && editingCampus.name.trim()) {
      const result = await updateCampus(editingCampus._id, { name: editingCampus.name, institutionName: '', municipality: editingCampus.municipality });
      if (result.success) {
        await fetchDropdownData();
  ...
  const handleVerifyCompliance = async () => {
    setIsChecking(true);
    setAiFeedback([]);
    const checklistName = 'Check Instalación Junta de Internet';
 ...
 const handleExportViabilityPDF = async () => {
    ...
    try {
      ...
      addHeaderAndFooter();
      const campusName = contractorInfo.campusName?.replace(/ /g, '_') || 'JuntaInternet';
      const dateStr = format(contractorInfo.date, 'yyyy-MM-dd');
      const fileName = `Concepto_Viabilidad_Junta_${campusName}_${dateStr}.pdf`;
      doc.save(fileName);
    ...
    try {
        ...
        if (result.success) {
          ...
          if (!form.getValues()._id) {
             ...
             setChecklistItems(checklists.junta);
             clearSignature();
             router.replace('/form/instalacion-junta', undefined);
          }
        } else {
          toast({
            variant: 'destructive',
  ...
  const handleExportCSV = () => {
    ...
    csvContent += `Junta de Internet,${contractorInfo.campusName}\n`;
  ...
  const handleExportPDF = async () => {
    ...
    try {
        ...
        const infoPairs = [
            { label: 'Junta de Internet:', value: contractorInfo.campusName },
               ...
               <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel>Nombre de la Junta de Internet</FormLabel>
                            ...
                            <CommandList>
                              <CommandEmpty>
                                <p className="p-4 text-sm text-muted-foreground">
                                  No se encontró la junta. Puede agregarla a la lista.
                                </p>
                              </CommandEmpty>
                              <CommandGroup>
                                {campuses.map((campus) => (
                      ...
                      <Dialog open={isManageCampusesOpen} onOpenChange={setIsManageCampusesOpen}>
                        <DialogTrigger asChild>
                          <Button variant="outline" size="icon" className="shrink-0">
                            ...
                            <span className="sr-only">Administrar Juntas</span>
                          </Button>
                        </DialogTrigger>
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>Administrar Juntas de Internet</DialogTitle>
                            <DialogDescription>
                              Agregue, edite o elimine juntas de la lista.
                            </DialogDescription>
                          </DialogHeader>
                          <div className="space-y-4">
                            <div className="flex flex-col gap-2">
                              <Input
                                ...
                                placeholder="Nombre de la nueva junta"
                                className="mb-2"
                              />
                              <Input
