// src/components/checklist-form-instalacion.tsx
'use client';
...
import { checklistInstalacionInstitucionEducativaData } from '@/lib/checklist-data';
...
const checklists = {
  institucion: checklistInstalacionInstitucionEducativaData.map((item) => ({
    ...item,
    observation: '',
    photoDataUri: null,
    status: 'na',
...
export function ChecklistFormInstalacion({ isViewer }: { isViewer: boolean }) {
  const { toast } = useToast();
  const searchParams = useSearchParams();
  const router = useRouter();
  
  const [checklistItems, setChecklistItems] = React.useState<ChecklistItem[]>(checklists.institucion);
  ...
  const handleVerifyCompliance = async () => {
    setIsChecking(true);
    setAiFeedback([]);
    const checklistName = 'Check Instalación Institución Educativa';
    ...
    try {
        ...
        if (result.success) {
          ...
          if (!form.getValues()._id) {
             ...
             setChecklistItems(checklists.institucion);
             clearSignature();
             router.replace('/form/instalacion', undefined);
          }
        } else {
          toast({
            variant: 'destructive',
               ...
               <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel>Nombre de la Sede Educativa</FormLabel>
                            ...
                            <CommandList>
                              <CommandEmpty>
                                <p className="p-4 text-sm text-muted-foreground">
                                  No se encontró la sede. Puede agregarla a la lista.
                                </p>
                              </CommandEmpty>
                              <CommandGroup>
                                {campuses.map((campus) => (
                      ...
                      <Dialog open={isManageCampusesOpen} onOpenChange={setIsManageCampusesOpen}>
                        <DialogTrigger asChild>
                          <Button variant="outline" size="icon" className="shrink-0">
                            ...
                            <span className="sr-only">Administrar Sedes</span>
                          </Button>
                        </DialogTrigger>
                        <DialogContent>
                          <DialogHeader>
                            <DialogTitle>Administrar Sedes Educativas</DialogTitle>
                            <DialogDescription>
                              Agregue, edite o elimine sedes de la lista.
                            </DialogDescription>
                          </DialogHeader>
